...
#define DESCRIPTOR_MODULE_NAME descriptor_all2_R
#endif
...
if (invalid_count > 0) {
    ...
    //                 radial1_feat0 ...]
    // --------------------------------------------------
    if (coef_py.size() == 0 || coef_py.size() < n_feat) {
        throw std::runtime_error("coef array must have size >= n_feat.");
    }
    auto coef = coef_py.unchecked<1>();
    std::cout << "checkpoint 1" << std::endl;

    py::array_t<double> np_e({n_atoms, 1});
    py::array_t<double> np_f({n_atoms * 3, 1});
    py::array_t<double> np_s({n_atoms * 9, 1});
    ...
    std::cout << "checkpoint 2: zeroed allocated arrays" << std::endl;
...
for (int center = 0; center < n_atoms; ++center) {
    ...
    for (int kfeat = 0; kfeat < n_feat; ++kfeat) {
        ...
        if (fm.valid) {
            double val = eval_feature_sparse_fast_multi(anlm_all, base, fm);
            e_sum += val * coef(kfeat);
        }
    }
    ptr_e(center, 0) = e_sum;
}
std::cout << "checkpoint 3: energy computed" << std::endl;
...
// Plan 1: pre-compute contracted lambda for all (atom, ir) into a dense buffer [n_atoms, nrad, NLM].
// The force loop uses dense dot products.
const size_t lambda_compact_size = static_cast<size_t>(n_atoms) * static_cast<size_t>(nrad) * static_cast<size_t>(NLM);
std::vector<double> lambda_compact(lambda_compact_size, 0.0);
std::cout << "checkpoint 4: allocated lambda_compact" << std::endl;
...
#pragma omp parallel
{
    ...
    for (int center = 0; center < n_atoms; ++center) {
        ...
        for (int kfeat = 0; kfeat < n_feat; ++kfeat) {
            ...
            const double c = coef(kfeat);
            const size_t dst_base =
                (static_cast<size_t>(center) * static_cast<size_t>(nrad)
                 + static_cast<size_t>(ir)) * static_cast<size_t>(NLM);
            for (int idx = 0; idx < NLM; ++idx) {
                lambda_compact[dst_base + idx] += c * lambda_buf[idx];
            }
        }
    }
}
...
const int nthreads = omp_get_max_threads();
// Fix 1: flat 1D layout [tid * n_atoms*3 + atom_alpha]
const size_t f_stride = static_cast<size_t>(n_atoms) * 3;
...
#pragma omp parallel
{
...
#else
    // Plan 1: lambda_compact is dense [n_atoms, nrad, NLM].
    ...
    for (size_t p = 0; p < n_pairs; ++p) {
        ...
        for (int ir = 0; ir < nrad; ++ir) {
            ...
            // Dense contracted lambda vectors from pre-computed buffer.
            const size_t lbase_i =
                (static_cast<size_t>(i) * static_cast<size_t>(nrad)
                 + static_cast<size_t>(ir)) * static_cast<size_t>(NLM);
            const size_t lbase_j =
                (static_cast<size_t>(j) * static_cast<size_t>(nrad)
                 + static_cast<size_t>(ir)) * static_cast<size_t>(NLM);
            const double* lambda_i = lambda_compact.data() + lbase_i;
            const double* lambda_j = lambda_compact.data() + lbase_j;

            // Dense dot products for the contracted dimension
            for (int idx = 0; idx < NLM; ++idx) {
        ...
        f_private[static_cast<size_t>(3 * j + 0)] += fij_x;
        f_private[static_cast<size_t>(3 * j + 1)] += fij_y;
        f_private[static_cast<size_t>(3 * j + 2)] += fij_z;
        f_private[static_cast<size_t>(3 * i + 0)] -= fij_x;
        f_private[static_cast<size_t>(3 * i + 1)] -= fij_y;
        f_private[static_cast<size_t>(3 * i + 2)] -= fij_z;
        f_private[static_cast<size_t>(3 * i + 0)] += fji_x;
        f_private[static_cast<size_t>(3 * i + 1)] += fji_y;
        f_private[static_cast<size_t>(3 * i + 2)] += fji_z;
        f_private[static_cast<size_t>(3 * j + 0)] -= fji_x;
        f_private[static_cast<size_t>(3 * j + 1)] -= fji_y;
        f_private[static_cast<size_t>(3 * j + 2)] -= fji_z;
    }
#endif
}
...
for (int t = 0; t < nthreads; ++t) {
    ...
    for (int atom_alpha = 0; atom_alpha < n_atoms * 3; ++atom_alpha) {
        ptr_f(atom_alpha, 0) += src_t[static_cast<size_t>(atom_alpha)];
    }
}
...
std::cout << "checkpoint 7: returning" << std::endl;
    ...
    switch (l_max_global) {
        case 0:
            return compute_all_descriptors_batch_impl<0>(
                pos_py, lat_py, n_param, all_ls_py, radial_params, r_c, compute_force, coef_py
            );
        case 1:
